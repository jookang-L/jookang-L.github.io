...
const DRAWER_LINKS = [
  ...
  { label: '교사용',   links: [{ href: '#greninja-section', text: '📊 생활기록부 분석' }, { href: '#infernape-section', text: '🔖 가장 편한 메모앱' }] },
  { label: '학생용',   links: [{ href: '#eevee-section', text: '🧠 기계학습 실습' }, { href: '#pandas-section', text: '🐼 pokepandas' }, { href: '#electrode-section', text: '🕹️ pygame' }, { href: '#aipom-section', text: '⚖️ 판사시스템' }] },
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div
            key="overlay"
            className="fixed inset-0 bg-black/70 z-[890] xl:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          ...
          <motion.div
            key="drawer"
            className="fixed top-0 right-0 h-full z-[900] xl:hidden overflow-y-auto"
            style={{
              width: 280,
              background: '#0f0f1e',
